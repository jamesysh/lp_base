...
struct KeyIndex_integral
{
  double key;//Should be a 32/64 bit integer. With a 32 bit integer we can use Octrees of depth 10. With a 64 bit integer we can use octrees of depth 20.
  int index;
  bool operator < (const KeyIndex_integral& a) const
  {
    return (key < a.key);
  }
};
...
private:
  ...
  */
  ...
  const double *m_vCoordX_integral;
  ...
  */
  ...
  KeyIndex *m_vParticleKeyIndex;
#endif
  KeyIndex_integral *m_vParticleKeyIndex_integral;
...
public:
  ...
  */
  ...
  int buildOctree(const double* x, const double* y, const double* z, const double* m, const double* vv, size_t numParticles, const double* x_integral) ;
...
private:
  ...
  int countNumberinRange(int n,double x, double y);
