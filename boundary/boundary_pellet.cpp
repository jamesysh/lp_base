...
int PelletInflowBoundary::UpdateInflowBoundary(ParticleData* m_pParticleData, EOS* m_pEOS, double dt, double dx){
    ...
    for(int pi=0;pi<pelletn;pi++){
		...
		{
			double tr=(x[i]-pelletx[pi])*(x[i]-pelletx[pi])+(y[i]-pellety[pi])*(y[i]-pellety[pi])+(z[i]-pelletz[pi])*(z[i]-pelletz[pi]);
			tr=fabs(sqrt(tr)-r);
			if(tr<dis)
			{	
				dis=tr;
				dx0=localParSpacing[i];
			}
		}
		double dr=5*dx0;
		for(size_t i=fluidStartIndex;i<inflowEndIndex;i++)
		{	
			...
			if(tr<dr)
			{
				double vr=vx[i]*(x[i]-pelletx[pi])+vy[i]*(y[i]-pellety[pi])+vz[i]*(z[i]-pelletz[pi]);
				vr=vr/sqrt((x[i]-pelletx[pi])*(x[i]-pelletx[pi])+(y[i]-pellety[pi])*(y[i]-pellety[pi])+(z[i]-pelletz[pi])*(z[i]-pelletz[pi]));
				massflowrate+=mass[i]*vr;
...
*/
         ...
         for(size_t index=fluidStartIndex;index<inflowEndIndex;index++)
		{
			...
			if(r<(pr+dx/10)*(pr+dx/10) &&  r>pir*pir)
			{   
...
/*
  ...
  */  
        ...
        for(size_t index = fluidEndIndex; index<inflowEndIndex; index++){
            ...
            {  
                volume[index] = volumeOnBoundary[pi];
                pressure[index] = R*Ts/volumeOnBoundary[pi];
   ...
   for(int i=0;i<layer_n;i++){
    ...
    for(int layer_id=0;layer_id<layer_n;layer_id++){
       ...
       for(int i=0;i<numberOfParticleOnLayer;i++){
           ...
           double dr = sqrt(d_x*d_x+d_y*d_y+d_z*d_z);
           vx[inflowEndIndex] = oldv*d_x/dr*dr*dr/pir/pir;
           vy[inflowEndIndex] = oldv*d_y/dr*dr*dr/pir/pir;
           vz[inflowEndIndex] = oldv*d_z/dr*dr*dr/pir/pir;
