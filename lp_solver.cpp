...
double upwind_end = omp_get_wtime();
printf("The time to solve LS system is %.5f\n.",upwind_end-upwind_start);
    if(!phase_success) {
		phase=0;
		//numParNotUpdate++;
...
//	cout<<"end building octree"<<endl;
	...
	size_t fluidEndIndex = fluidStartIndex + m_pParticleData->getFluidNum() + m_pParticleData->m_iInflowNum;
	size_t inflowEndIndex = fluidEndIndex + m_pParticleData->getInflowNum();
...
//end of sph density estimator
		...
		for(size_t index=fluidStartIndex; index<fluidEndIndex; index++) { 
            ...
            if(index>=fluidEndIndex){
                pi = pelletid[index];
                double d_x = positionX[index] - pelletx[pi];
                double d_y = positionY[index] - pellety[pi];
                double d_z = positionZ[index] - pelletz[pi];
                double r = sqrt(d_x*d_x + d_y*d_y + d_z*d_z);
                radius *= r*r/pelletir[pi]/pelletir[pi];
            }
           // cout<<"index "<<index<<" radius  "<<radius<<" pi "<<pi<<endl;
...
int cap = m_pParticleData->m_iFluidNum + m_pParticleData->m_iInflowNum;
...
// printf("In %d compute result for number %d is %.6f,%.6f\n",m_iCount,index,result[0],result[1]);
...
// printf("In %d compute p_d for number %d is %.6f\n",m_iCount,index,*p_d);

*p_dd = LPFOrder[index]==2? result[dir+offset]/distance/distance:0;
// printf("In %d compute p_dd for number %d is %.6f\n",m_iCount,index,*p_dd);
...
// printf("In %d compute result for number %d is %.6f,%.6f\n",m_iCount,index,result[0],result[1]);
  ...
  //          printf("In %d compute vel_d for number %d is %.6f\n",m_iCount,index,*vel_d);
            *vel_dd = LPFOrder[index]==2? result[dir+offset]/distance/distance:0;
	//		 printf("In %d compute vel_dd for number %d is %.6f\n",m_iCount,index,*vel_dd);
