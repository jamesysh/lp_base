...
//-------------------SETUP OUTPUT OPTION------------------------
    ...
    result = find(output.begin(),output.end(),"volume");
  ...
  if(m_iPelletDistribution != 0){ 
   ...
   result = find(output.begin(),output.end(),"leftintegral");

    if(result != output.end())
        m_iPrintLeftIntegral = 1;
    
    result = find(output.begin(),output.end(),"rightintegral");

    if(result != output.end())
        m_iPrintRightIntegral = 1;
    
    result = find(output.begin(),output.end(),"deltaq");

    if(result != output.end())
        m_iPrintDeltaq = 1;
     
    result = find(output.begin(),output.end(),"qplusminus");

    if(result != output.end())
        m_iPrintQplusminus = 1;
...
void Initializer::initPellet(){
	...
	m_iMaxParticlePerCell = 2000;//maximum number of particles per APCloud node in density integral calculation
