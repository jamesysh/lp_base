...
int Octree::buildOctree(const double* x, const double* y, const double* z,const double* m, const double* vv,  size_t numParticles, const double* x_integral){
    
    m_vCoordX_integral = x_integral;
    buildOctree(x,y,z,numParticles);
    mass = m;
    VolumeVoronoi = vv;
    return 0;
...
void Octree::computeIntegralQuadtree(const double* mass, double* left_integral, double* right_integral, int fluidnum, int maxnp, int binaryresolution){

    
    int maxNumberPerCell = binaryresolution;
...
//    std::vector<double> left_sum(maxCellNumberInBox);
  //  std::vector<double> right_sum(maxCellNumberInBox);
  //  std::vector<double> local_sum(maxCellNumberInBox) ;
    ...
    cout<<"Generated "<<n<<" Quadtree computational nodes, max level = "<<deepest<<endl;
	if(flag==true)
   ...
   for(int i=0; i<n; i++)
   {
	if(child[i] == -1)
	{
		integral_queue.push(i);
            
        } 
       
   }
  
    double x_min = *std::min_element(m_vCoordX_integral,m_vCoordX_integral + m_iTotalNumberOfParticles);
    double x_max = *std::max_element(m_vCoordX_integral,m_vCoordX_integral + m_iTotalNumberOfParticles);
    double x_center;
    double x_lower = x_min, x_upper = x_max;
   ...
   {   
       ...
       int numberofParticleInBox = m_vNumberOfContainedParticles[inode];
       if (numberofParticleInBox == 0)
           continue;
      
       double y_length = y_size[l];
       ...
       */
       for(int k=m_vFirstParticleIndex[inode],id_particle=0;k<m_vFirstParticleIndex[inode] + numberofParticleInBox;k++,id_particle++)
       {
            ...
            m_vParticleKeyIndex_integral[id_particle].index = j;
            m_vParticleKeyIndex_integral[id_particle].key = m_vCoordX_integral[j];
           
    } 
    std::sort(m_vParticleKeyIndex_integral, m_vParticleKeyIndex_integral+numberofParticleInBox);
  
//-------------------binary tree----------------------------------
    std::vector<int> cell_ID;
    std::vector<double> x_left;
    std::vector<double> x_right;
    std::vector<double> x_dividing;
    cell_ID.push_back(0);
    ...
    int i_node=0;
    int i_nodeend = 1;
    int np_node = numberofParticleInBox;
   
   while(i_node<i_nodeend){
        
        int node_id = cell_ID[i_node];
        x_lower = x_left[node_id];
        x_upper = x_right[node_id];
        np_node = countNumberinRange(numberofParticleInBox,x_lower,x_upper);
        
        if(np_node <= maxNumberPerCell)
    ...
    std::sort(x_dividing.begin(),x_dividing.end());
    int division = x_dividing.size();
    
   	std::vector<double> left_sum(division,0);
...
int Octree::countNumberinRange( int n, double x, double y) 
    { 
        int l = 0, h = n - 1; 
        while (l <= h) { 
            int mid = (l + h) / 2; 
            if (m_vParticleKeyIndex_integral[mid].key >= x) 
                h = mid - 1; 
            else
               l = mid + 1; 
                       } 
       ...
       h = n-1;
         ...
         { 
            int mid = (l + h) / 2; 
            if (m_vParticleKeyIndex_integral[mid].key <= y) 
                l = mid + 1; 
            else  
                h = mid - 1;       
                }
